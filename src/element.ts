...
export abstract class LoomElement extends HTMLElement {
  ...
  connectedCallback(): void {
    ...
    for (const handler of (this as any)[ON_HANDLERS] ?? []) {
      if (handler.domTarget) {
        // DOM EventTarget: @on(window, "resize")
        const fn = (e: Event) => (this as any)[handler.key](e);
        ...
        this.track(bus.on(handler.type, (e: any) => (this as any)[handler.key](e)));
      }
    }
    ...
    // Fire @mount handlers
    for (const key of (this as any)[MOUNT_HANDLERS] ?? []) {
      (this as any)[key]();
    }
  }
  ...
  disconnectedCallback(): void {
    ...
    for (const key of (this as any)[UNMOUNT_HANDLERS] ?? []) {
      (this as any)[key]();
    }
